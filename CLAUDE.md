...
每一个进入这个项目的 Claude 必须把以下三条根植内心，**违反 = 协作失败**，不是工程瑕疵：

### 1. 每一个视觉决策都必须能讲出文化锚点

不能用"现代极简"、"温暖纸感"这种 AI 通用美学。山是范宽 / 马远 / 富春山居图 / 杉本博司；星是 Cosmic Latte（#FFF8E7，2003 年 Glazebrook & Baldry 用 2dF 巡天 20 万星系算出的宇宙平均色）；楷字是 Ma Shan Zheng 手札体；卡片倾斜的 ±1.2° 是"纸张落桌"的物理感。每一处都能讲出"为什么是这个不是别的"的故事。

**新方向第一句话必须给出锚点 —— 没有锚点的方案不进 demo。**

### 2. Less is more 是字面意思 —— 克制 = 融合到位，不是减少功能

搜索功能从摆个 icon → 当前下拉揭示 PullSearch 岛屿，迭代了多版。每一版都在问"功能能不能再融化进当前氛围一点"，直到答案是"它本来就该长这样"。一上来就摆个搜索框是工程师做法，不是这个项目的做法。

**加新功能时，先问：能不能化在已有氛围里？答案是"勉强"就再迭代。**

### 3. 氛围优先于效率

慢写慢读的页面，不是产品页。每一个滚动状态、每一个空状态、每一个分隔符都要有意境，不是工程默认。"用户能完成任务"不是这个站的成功标准 —— 用户进来感受到一抹蓝、一笔墨、一段静，才是。

**完整论述、具体正反例与历史教训：DESIGN_RULES.md R0**
