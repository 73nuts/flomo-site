...
## R0 · 设计基因（meta-rule，所有规则之上）

> "视觉先于功能，设计规范先于代码"。这条不是品味偏好，是项目存在的前提。
...
**正例**：
- 山水 = 范宽《溪山行旅图》/ 马远《踏歌图》/ 黄公望《富春山居图》/ 杉本博司《海景》—— 远山轮廓 + 5 段山色协同（R14）
- 星点 = Cosmic Latte `#FFF8E7`（2003 年 Glazebrook & Baldry 用 2dF 巡天 20 万星系算出的宇宙平均色，R13）
- 楷字 = 汪曾祺手札 / 董桥 / Ma Shan Zheng（R15）
- 卡片倾斜 = ±1.2° 纸张落桌物理感（R3）
- 第 N 次访问签名字 = 手抄日记习惯（R5）

**反例**（曾经被否决的描述用语）：
- "现代极简" / "温暖纸感" / "high contrast premium feel" / "scandinavian minimal" / "modern clean"
- 这些是 AI 通用美学，用户能立即识别并否决

**判定**：方案第一句不能给出具体文化 / 历史 / 文献锚点 → 不进 demo，回炉。

#### 二、Less is more 是字面意思 —— 克制 = 融合到位

**范例：搜索功能的迭代史**

最早的版本是顶部一个 magnifier icon。被否决：icon 突兀，跟纸本氛围打架。中间经历多版（内嵌搜索框 / 入口加搜索 tab / 浮动搜索按钮等），每一版都"看得出来是后加的"，每一版都被否决。当前形态：**下拉揭示 PullSearch 岛屿** —— 列表页拉到顶后再往下拽，搜索浮出；松手回弹消失。看起来这站本来就有这个，找不出"被加上去"的痕迹。

**评判标准**：新功能加进来如果"看得出来是后加的"，回炉重做。**能让用户"以为这站本来就有这个"才合格。**

#### 三、氛围优先于效率

这是慢写慢读的页面，**不是产品页**。

- 滚动状态 → 必须有意境（sky 如何"告别" / "隐居" / "退潮"，不是 fade-out 默认）
- 空状态 → 必须有故事（"糟糕，阿鸭还没写过这句"）
- 分隔符 → 必须像纸本（28px 单根短横，不是 div border）
- 加载态 → 不存在（静态 0 JS，根本没 loading）

**判定**：选 A 比 B 更"好用"但 B 更"有氛围"时，**选 B**。

### 违反 R0 = 协作失败，不是工程瑕疵

历史教训（用户多次纠正）：
- 直接动 `src/` 不走 `frontend-design` skill → 视觉越调越偏
- 用 AI 通用美学形容方向（"温暖"、"现代"、"高级感"）→ 立即被识别并否决
- 把功能"加上"而不是"融进" → 整体氛围被打断
- 凭工程逻辑推演视觉方案（"Path A vs B vs C"）→ 用工程框架代替设计判断

这些都不是"代码 bug"，是"协作契约违反"。修复路径不是"再调一下代码"，而是**回到 R0 重新理解项目**。
